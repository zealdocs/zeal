...
QString stripParameterUrl(const QString &url, const QString &scheme) {
  QStringView ref = QStringView{url}.mid(scheme.length() + 1);
...
CommandLineParameters parseCommandLine(const QStringList &arguments) {
  ...
  parser.setApplicationDescription(
      QObject::tr("Zeal - Offline documentation browser."));
  ...
  clParams.unregisterProtocolHandlers =
      parser.isSet(QStringLiteral("unregister"));

  if (clParams.registerProtocolHandlers &&
      clParams.unregisterProtocolHandlers) {
    QTextStream(stderr) << QObject::tr(
        "Parameter conflict: --register and --unregister.\n");
    ::exit(EXIT_FAILURE);
  }
#endif
  ...
  if (arg.startsWith(QLatin1String("dash:"))) {
    ...
    const QUrlQuery urlQuery(
        stripParameterUrl(arg, QStringLiteral("dash-plugin")));
...
#ifdef Q_OS_WIN32
void registerProtocolHandler(const QString &scheme,
                             const QString &description) {
  const QString appPath =
      QDir::toNativeSeparators(QCoreApplication::applicationFilePath());
  const QString regPath =
      QStringLiteral("HKEY_CURRENT_USER\\Software\\Classes\\") + scheme;

  QScopedPointer<QSettings> reg(
      new QSettings(regPath, QSettings::NativeFormat));
  ...
  reg->setValue(QStringLiteral("Default"),
                QVariant(appPath + QLatin1String(" %1")));
}

void registerProtocolHandlers(const QHash<QString, QString> &protocols,
                              bool force = false) {
  const QString regPath =
...
int main(int argc, char *argv[]) {
  ...
  QScopedPointer<Core::ApplicationSingleton> appSingleton(
      new Core::ApplicationSingleton());
  ...
  // Set application-wide window icon. All message boxes and other windows will
  // use it by default.
  ...
  QDir::setSearchPaths(QStringLiteral("typeIcon"),
                       {QStringLiteral(":/icons/type")});

  QScopedPointer<Core::Application> app(new Core::Application());

  QObject::connect(appSingleton.data(),
                   &Core::ApplicationSingleton::messageReceived,
